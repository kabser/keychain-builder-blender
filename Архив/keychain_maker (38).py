...
def weld_ear_to_base(context, base_obj, ear_obj, right_x, y0, y1):
    ...
    # Убираем лишние копланарные рёбра внутри плоского меша
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        # ── 5. Подготовка плоского меша перед экструзией ─────────────────
        # Закрываем все открытые петли, убираем внутренние лишние рёбра
        set_active(context, base_flat)
        bpy.ops.object.editmode_toggle()
        bm_b = bmesh.from_edit_mesh(base_flat.data)

        # Несколько проходов fill для закрытия всех петель
        for _ in range(5):
            open_e = [e for e in bm_b.edges if e.is_boundary]
            if not open_e:
                break
            bmesh.ops.triangle_fill(bm_b, use_beauty=True, edges=open_e)
