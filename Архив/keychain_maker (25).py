...
# ---------------------------------------------------------------------------
# Ушко (строится ПОСЛЕ transform_apply когда Z финальные)
# ---------------------------------------------------------------------------

def build_lug_3d(context, final_obj, props):
    """
    Строит ушко как отдельный 3D объект.

    Вызывается ПОСЛЕ того как final_obj уже поднят (transform_apply применён),
    то есть нижняя грань = Z=0, верхняя = Z=base_h.

    Координаты берём из фактических вершин final_obj.
    """
    ...
    # Читаем реальные координаты объекта в мировом пространстве
    ...
    z_bot      = min(zs)   # нижняя грань объекта
    z_top      = max(zs)   # верхняя грань объекта

    # Координаты ушка
    x1 = base_min_x        # правый край вплотную к подложке
    ...
    bm.faces.new([bot[3], bot[2], bot[1], bot[0]])   # низ (нормаль вниз)
    bm.faces.new([top[0], top[1], top[2], top[3]])   # верх (нормаль вверх)
    bm.faces.new([bot[0], bot[1], top[1], top[0]])   # перед
    bm.faces.new([bot[1], bot[2], top[2], top[1]])   # право (примыкает к подложке)
    bm.faces.new([bot[2], bot[3], top[3], top[2]])   # зад
    bm.faces.new([bot[3], bot[0], top[0], top[3]])   # лево (внешняя сторона)
    bmesh.ops.recalc_face_normals(bm, faces=bm.faces[:])
    bm.to_mesh(mesh)
    bm.free()

    # Фаска на двух левых вертикальных рёбрах (сторона x0)
    if chamfer > 0.0:
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        # Теперь final_obj в финальных координатах:
        # низ = Z=0, верх подложки = Z=base_h, верх букв = Z=base_h+letter_h
        if props.lug_enable:
...
class KeychainProperties(PropertyGroup):
    ...
    lug_enable: BoolProperty(
        name="Добавить ушко", default=True,
    )
