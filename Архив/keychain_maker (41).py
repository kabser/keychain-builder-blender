...
def weld_ear_pdf_method(context, base_obj, ear_obj, right_x, y0, y1):
    """
    ...
    try:
        ...
        # (основание правее ушка, значит нужно двигать вправо)
        extrude_dist = 5.0  # достаточно чтобы гарантированно пересечь
        ...
        ear_right = [v for v in bm.verts
                     if abs(v.co.x - right_x) < tol_x
                     and (y0 - tol_y) < v.co.y < (y1 + tol_y)]

        # Находим крайние Y новых вершин на основании
        # Они должны быть правее right_x и иметь Y в нашем диапазоне
        base_region = [v for v in bm.verts
                       if v.co.x > right_x + 0.01
                       and (y0 - tol_y) < v.co.y < (y1 + tol_y)]
...
def cut_ear_hole(context, obj, outer_x, yc, base_h, hole_d):
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        if props.lug_enable:
            ...
            # Ушко с зазором 0.5мм чтобы Auto Merge + Split сработал
            gap     = 0.5
            ...
            # Вшиваем ушко в подложку методом из PDF
