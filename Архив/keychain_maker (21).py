...
def build_lug(context, base_obj, props):
    """
    Строит ушко (lug) с левой стороны подложки.

    Геометрия:
    - Прямоугольник размером lug_x × lug_y в плоскости XY
    - Высота = base_height (совпадает с подложкой)
    - Расположен слева от подложки (X = base_min_x - lug_x .. base_min_x)
    - Смещён по Y относительно центра подложки на lug_offset_y
    - На четырёх вертикальных рёбрах (по Z) — фаска chamfer
    - Отверстие диаметром hole_d, центр на расстоянии hole_d/2 + 2мм от левого края

    Координаты строятся в локальном пространстве Z=0..base_height,
    потом объект сдвигается как финальный брелок.
    """
    ...
    hole_d     = props.lug_hole_diameter
    hole_r     = hole_d / 2.0
    seg        = 32  # сегменты окружности отверстия

    # Clamp фаски: не больше половины меньшей стороны
    chamfer = min(chamfer, lug_x * 0.45, lug_y * 0.45)

    # Находим левую границу подложки по X
    verts_x = [v.co.x for v in base_obj.data.vertices]
    base_min_x = min(verts_x)

    # Находим центр подложки по Y
    verts_y = [v.co.y for v in base_obj.data.vertices]
    base_cy = (min(verts_y) + max(verts_y)) / 2.0

    # Координаты ушка в плоскости Z=0 (верх)
    # X: от (base_min_x - lug_x) до base_min_x
    # Y: центрировано по base_cy + offset_y
    x0 = base_min_x - lug_x
    x1 = base_min_x
    y0 = base_cy + offset_y - lug_y / 2.0
    y1 = base_cy + offset_y + lug_y / 2.0

    # Центр отверстия
    # От левого края (x0) отступаем: hole_r + 2мм
    hole_cx = x0 + hole_r + 2.0
    hole_cy = (y0 + y1) / 2.0

    # --- строим меш ушка через bmesh ---
    bm = bmesh.new()

    # 8 вершин прямоугольника с фасками на углах (вид сверху, Z=0)
    # Обходим против часовой стрелки для нормали вверх
    #
    #  (x0,y1-c) -- (x0+c,y1) -- (x1-c,y1) -- (x1,y1-c)
    #      |                                        |
    #  (x0,y0+c)                              (x1,y0+c)
    #      |                                        |
    ...
    # Создаём верхние вершины (Z = base_h) и нижние (Z = 0)
    top = [bm.verts.new(Vector((x, y, base_h))) for x, y in top_verts_2d]
    bot = [bm.verts.new(Vector((x, y, 0.0)))    for x, y in top_verts_2d]
    ...
    # Отверстие: вырезаем цилиндр через Boolean
    # Сначала создаём меш ушка без отверстия, потом применяем Boolean
    bmesh.ops.recalc_face_normals(bm, faces=bm.faces[:])

    lug_mesh = bpy.data.meshes.new("_KM_LugMesh")
    bm.to_mesh(lug_mesh)
    bm.free()

    lug_obj = bpy.data.objects.new("_KM_Lug", lug_mesh)
    context.collection.objects.link(lug_obj)

    # --- цилиндр для отверстия ---
    ...
    mod = lug_obj.modifiers.new("CutHole", "BOOLEAN")
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        # ── 6. Ушко ────────────────────────────────────────────────────────
        lug_obj = build_lug(context, base_obj, props)
...
class KeychainProperties(PropertyGroup):
    ...
    lug_chamfer: FloatProperty(
        name="Фаска рёбер (мм)",
        description="Размер фаски на вертикальных рёбрах ушка",
    ...
    lug_hole_diameter: FloatProperty(
        ...
        description="Диаметр отверстия для кольца в ушке",
