...
def prepare_letters_mesh(bm):
    """
    Из меша букв оставляет только контуры, заполняет с дырками.
    Гарантирует что все нормали смотрят вверх (+Z).
    """
    L(f"  letters input: verts={len(bm.verts)}, edges={len(bm.edges)}, faces={len(bm.faces)}")

    # Граничные рёбра = контуры букв и дырок
    ...
    # КЛЮЧЕВОЕ: принудительно все нормали вверх
    # recalc_face_normals только согласовывает между собой — не гарантирует +Z
    # flip_faces_down переворачивает конкретно те что смотрят вниз
    flip_faces_down(bm)

    L(f"  letters готово: verts={len(bm.verts)}, faces={len(bm.faces)}")
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        mod_let.offset          = 1.0    # экструзия в направлении нормали (+Z)
        mod_let.use_even_offset = True
        mod_let.use_rim         = True
        bpy.ops.object.modifier_apply(modifier="Solidify")

        L(f"  буквы после solidify: verts={len(letters_obj.data.vertices)}, faces={len(letters_obj.data.polygons)}")
