...
class KLD_OT_Step5(Operator):
    ...
    def execute(self, context):
        ...
        # Собираем пары: (вершина ушка, новая вершина на контуре подложки)
        pairs = []
        ...
        for ear_v in right_verts:
            ...
            va_v = best_edge.verts[0]
            vb_v = best_edge.verts[1]
            new_v = bm.verts.new(Vector((best_ix, ey, 0.0)))
            bm.edges.remove(best_edge)
            try: bm.edges.new([va_v, new_v])
            except Exception: pass
            try: bm.edges.new([new_v, vb_v])
            except Exception: pass

            pairs.append((ear_v, new_v))
            bm.verts.ensure_lookup_table()
            bm.edges.ensure_lookup_table()

        created = len(pairs)
        print(f"[KLD Step5] pairs={created}")

        # Строим грань соединения явно из 4 точек
        # Порядок: нижний_ушко → нижний_подложка → верхний_подложка → верхний_ушко
        if len(pairs) == 2:
            # pairs отсортированы по Y (нижняя, верхняя) через sort выше
            ear_bot, base_bot = pairs[0]
            ear_top, base_top = pairs[1]
            if all(v.is_valid for v in [ear_bot, base_bot, ear_top, base_top]):
                try:
                    bm.faces.new([ear_bot, base_bot, base_top, ear_top])
                    print("[KLD Step5] quad face created OK")
                except Exception as e:
                    print(f"[KLD Step5] quad failed: {e}, trying triangles")
                    try: bm.faces.new([ear_bot, base_bot, base_top])
                    except Exception: pass
                    try: bm.faces.new([ear_bot, base_top, ear_top])
                    except Exception: pass

        # Заполняем оставшиеся открытые петли (контур ушка + контур подложки)
        bmesh.ops.remove_doubles(bm, verts=bm.verts[:], dist=0.001)
        bm.edges.ensure_lookup_table()
        for _ in range(3):
