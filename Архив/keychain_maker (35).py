...
class KLD_OT_Step5(Operator):
    bl_idname  = "kld.step5"
    bl_label   = "Шаг 5: Диагностика 4 точек (только лог)"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        # Читаем меш безопасно — без editmode
        bm = bmesh.new()
        bm.from_mesh(joined.data)
        ...
        print("
[KLD Step5] === 4 ТОЧКИ СОЕДИНЕНИЯ ===")
        print(f"  right_verts: {len(right_verts)}")

        results = []
        for ear_v in right_verts:
            ex, ey = ear_v.co.x, ear_v.co.y
            print(f"  ear_v = ({ex:.4f}, {ey:.4f})")
            ...
            if best_ix is not None:
                bm.edges.ensure_lookup_table()
                e = bm.edges[best_ei]
                print(f"    base_v = ({best_ix:.4f}, {best_iy:.4f})")
                print(f"    ребро #{best_ei}: "
                      f"({e.verts[0].co.x:.3f},{e.verts[0].co.y:.3f})-"
                      f"({e.verts[1].co.x:.3f},{e.verts[1].co.y:.3f})")
                results.append((ex, ey, best_ix, best_iy, best_ei))
            else:
                print(f"    НЕТ пересечения!")
  ...
  КВАД:")
            ...
            print(f"    base_bot = ({r0[2]:.3f}, {r0[3]:.3f})")
            print(f"    base_top = ({r1[2]:.3f}, {r1[3]:.3f})")
            print(f"    ear_top  = ({r1[0]:.3f}, {r1[1]:.3f})")
            print(f"  Ширина соединения: {r0[2]-r0[0]:.3f} и {r1[2]-r1[0]:.3f} мм")
            print(f"  Высота соединения: {r1[1]-r0[1]:.3f} мм")

        self.report({"INFO"},
            f"Найдено пар: {len(results)} — смотрите консоль")
        return {"FINISHED"}
