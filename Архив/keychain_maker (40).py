...
def build_flat_base(context, pts, offset):
    ...
    # Убираем лишние внутренние рёбра после triangle_fill
...
# ---------------------------------------------------------------------------
# Ушко: отдельный 3D объект (v25 — проверенный подход)
# ---------------------------------------------------------------------------
...
def build_lug_3d(context, final_obj, props):
    ...
    # Ушко от Z=0 до Z=base_h
    z_bot = 0.0
    z_top = base_h

    # Правый край заходит в подложку
    ...
    bm.faces.new([bot[0], bot[1], top[1], top[0]])  # фронт
    bm.faces.new([bot[1], bot[2], top[2], top[1]])  # правый (в подложку)
    bm.faces.new([bot[2], bot[3], top[3], top[2]])  # зад
    bm.faces.new([bot[3], bot[0], top[0], top[3]])  # левый (внешний)
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        # ── 6. Join подложки и букв → поднимаем ────────────────────────────
        deselect_all()
        ...
        if props.lug_enable:
            ...
            # Boolean Union: сливаем ушко с брелоком в единый меш
            set_active(context, final_obj)
            mod = final_obj.modifiers.new("LugUnion", "BOOLEAN")
            mod.operation   = "UNION"
            mod.object      = lug_obj
            mod.solver      = "EXACT"
            bpy.ops.object.modifier_apply(modifier="LugUnion")
            delete_obj(lug_obj)
