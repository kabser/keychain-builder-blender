...
def smooth_contour(bm, iterations, factor=0.5):
...
def build_flat_base(context, pts, offset, smooth_iter):
    ...
    Верхняя грань подложки лежит на Z=0.
    После экструзии вниз: подложка занимает Z = [-base_h .. 0].
    Буквы начинаются от Z=0 и идут до Z=+letter_h.
    """
    ...
    L(f"  подложка: verts={len(bm.verts)}, faces={len(bm.faces)}")
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        # ── 3. Плоский контур подложки (верх на Z=0) ───────────────────────
        ...
        # ── 4. Экструдируем подложку ВНИЗ на base_h ────────────────────────
        # Результат: подложка от Z=-base_h до Z=0
        # Верхняя поверхность подложки = Z=0
        ...
        # ── 5. Буквы: чистим + Solidify вверх от Z=0 ───────────────────────
        # Буквы лежат на Z=0 (верх подложки) и экструдируются вверх до Z=+letter_h
        # letters_obj.location = (0,0,0) — буквы уже на нужной высоте
        ...
        # Центрируем по Z: сейчас объект от -base_h до +letter_h
        # Опускаем на Z=0 (нижняя грань касается пола)
