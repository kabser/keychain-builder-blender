...
def build_flat_base(context, pts, offset, mb_resolution):
    """
    Строит плоский замкнутый меш-контур (Z=0) через метаболы.

    Ключевое исправление: после сплющивания метабола в Z=0
    НЕ удаляем грани вручную. Вместо этого:
    1. Делаем merge by distance — схлопываем совпавшие вершины
    2. Удаляем грани с нулевой площадью (degenerate)
    3. Удаляем рёбра с нулевой длиной
    4. После этого is_boundary работает корректно
    5. Удаляем внутренние рёбра (не граничные)
    6. Заполняем контур holes_fill
    """
    # --- создаём метаболы ---
    ...
    # --- сплющиваем в Z=0 через bmesh (не через scale!) ---
    # scale.z=0 + transform_apply оставляет все вершины с z=0,
    # но не объединяет их — поэтому is_boundary не работает.
    # Правильный путь: принудительно выставить z=0 и сделать merge.
    set_active(context, base_raw)
    bpy.ops.object.editmode_toggle()
    bm = bmesh.from_edit_mesh(base_raw.data)

    # Принудительно ставим Z=0 для всех вершин
    for v in bm.verts:
        v.co.z = 0.0

    # Merge by distance — схлопываем все совпавшие вершины
    # После сплющивания очень много дублей — используем агрессивный порог
    bmesh.ops.remove_doubles(bm, verts=bm.verts[:], dist=0.1)

    # Удаляем вырожденные грани (нулевая площадь) и рёбра (нулевая длина)
    bmesh.ops.dissolve_degenerate(bm, dist=0.01, edges=bm.edges[:])

    # Теперь is_boundary работает корректно:
    # граничное ребро = принадлежит ровно одной грани
    # Удаляем внутренние рёбра (принадлежащие двум и более граням)
    interior = [e for e in bm.edges if not e.is_boundary]
    bmesh.ops.delete(bm, geom=interior, context="EDGES")

    # Удаляем осиротевшие вершины
    isolated = [v for v in bm.verts if not v.link_edges]
    bmesh.ops.delete(bm, geom=isolated, context="VERTS")

    # Заполняем каждый замкнутый контур
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        bm4 = bmesh.from_edit_mesh(base_flat.data)
        bot_edges = [e for e in bm4.edges
                     if e.is_boundary and all(v.co.z < -(base_h * 0.5) for v in e.verts)]
        if bot_edges:
            bmesh.ops.holes_fill(bm4, edges=bot_edges, sides=0)
        ...
        # ── 5. Буквы: чистим топологию + Solidify ──────────────────────────
        ...
        # Заполняем контуры (внешние + дырки букв e, g, о…)
        bound5 = [e for e in bm5.edges if e.is_boundary]
        if bound5:
            bmesh.ops.holes_fill(bm5, edges=bound5, sides=0)
...
class KeychainProperties(PropertyGroup):
    text: StringProperty(
        name="Текст", default="IVAN",
    )
    font_path: StringProperty(
        name="Шрифт (.ttf)", subtype="FILE_PATH", default="",
    )
    base_offset: FloatProperty(
        name="Отступ подложки (мм)",
        default=3.0, min=0.5, max=30.0, unit="LENGTH",
    )
    base_height: FloatProperty(
        name="Толщина подложки (мм)",
        default=3.0, min=0.5, max=20.0, unit="LENGTH",
    )
    letter_height: FloatProperty(
        name="Высота букв (мм)",
        default=2.0, min=0.2, max=10.0, unit="LENGTH",
    )
