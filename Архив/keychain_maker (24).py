...
# ---------------------------------------------------------------------------
# Построение плоского контура подложки (v20, проверенная версия)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Подготовка меша букв (v20, проверенная версия)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Ушко: отдельный 3D объект
# ---------------------------------------------------------------------------

def build_lug_3d(context, base_obj, props, base_h):
    """
    Строит ушко как самостоятельный 3D объект.

    Координаты:
    - Ушко вплотную к левому краю подложки
    - Z: от 0 до base_h (совпадает с подложкой)
    - Фаска на четырёх вертикальных рёбрах через Bevel modifier
    - Отверстие через Boolean

    Возвращает готовый объект ушка.
    """
    ...
    # Координаты ушка
    x1 = base_min_x          # правый край = левый край подложки
    x0 = x1 - lug_x          # левый край
    yc = base_cy + offset_y
    y0 = yc - lug_y / 2.0
    y1 = yc + lug_y / 2.0

    # ── Строим параллелепипед через bmesh ──────────────────────────────────
    ...
    # 8 вершин: низ (z=0) и верх (z=base_h)
    bot = [
    ...
    top = [
        bm.verts.new((x0, y0, base_h)),
        bm.verts.new((x1, y0, base_h)),
        bm.verts.new((x1, y1, base_h)),
        bm.verts.new((x0, y1, base_h)),
    ]

    # Грани
    bm.faces.new([bot[0], bot[1], bot[2], bot[3]])           # низ
    bm.faces.new([top[3], top[2], top[1], top[0]])           # верх
    bm.faces.new([bot[0], top[0], top[1], bot[1]])           # перед Y-
    bm.faces.new([bot[1], top[1], top[2], bot[2]])           # право X+
    bm.faces.new([bot[2], top[2], top[3], bot[3]])           # зад Y+
    bm.faces.new([bot[3], top[3], top[0], bot[0]])           # лево X-

    bmesh.ops.recalc_face_normals(bm, faces=bm.faces[:])
    bm.to_mesh(mesh)
    bm.free()

    # ── Фаска только на ЛЕВЫХ вертикальных рёбрах (X=x0) ──────────────────
    # Правые рёбра (X=x1) примыкают к подложке — их не скругляем
    if chamfer > 0.0:
        ...
        for e in bm2.edges:
            v0c, v1c = e.verts
            # Вертикальное ребро: оба конца на x0, разные Z
            is_vertical_left = (
                abs(v0c.co.x - x0) < tol and abs(v1c.co.x - x0) < tol and
                abs(v0c.co.y - v1c.co.y) < tol
            )
            e.select = is_vertical_left
    ...
    # ── Отверстие через Boolean ────────────────────────────────────────────
    # Центр по X: от левого края (x0 после фаски ≈ x0) + hole_r + 2мм
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        lug_obj = None
        if props.lug_enable:
            lug_obj = build_lug_3d(context, base_obj, props, base_h)
...
class KeychainProperties(PropertyGroup):
    ...
    lug_chamfer: FloatProperty(
        name="Фаска рёбер (мм)",
        description="Фаска на левых вертикальных рёбрах ушка",
        default=1.5, min=0.0, max=5.0, precision=1, step=5,
    )
    lug_chamfer_segments: IntProperty(
        name="Сегменты фаски",
        description="Кол-во сегментов фаски (1=прямая, 4+=скругление)",
