...
class KeychainProperties(PropertyGroup):
    ...
    char_spacing: FloatProperty(
        name="Расстояние между символами",
        description="Межсимвольный интервал (1.0 = стандартный, меньше = теснее)",
...
class KEYCHAIN_OT_Reset(Operator):
    ...
    bl_description = "Вернуть все параметры к значениям по умолчанию"
    bl_options = {"REGISTER", "UNDO"}
...
class KEYCHAIN_PT_Panel(Panel):
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.operator("keychain.reset_settings", icon="LOOP_BACK", text="Сбросить настройки")
