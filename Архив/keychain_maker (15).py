...
def flip_faces_down(bm):
    """Переворачивает грани у которых нормаль смотрит вниз (Z < 0)."""
    to_flip = [f for f in bm.faces if f.normal.z < 0]
    if to_flip:
        bmesh.ops.reverse_faces(bm, faces=to_flip)


def smooth_contour(bm, iterations=5, factor=0.5):
    """
    Сглаживает вершины граничного контура (Laplacian-style).
    Каждая вершина смещается к среднему положению своих соседей
    вдоль контура. Делаем только в XY, Z не трогаем.
    """
    # Собираем только граничные вершины (те у которых есть boundary ребро)
    boundary_verts = set()
    for e in bm.edges:
        if e.is_boundary:
    ...
    boundary_verts = list(boundary_verts)
    L(f"  сглаживание контура: вершин={len(boundary_verts)}, iter={iterations}")
    ...
    for _ in range(iterations):
        ...
        for v in boundary_verts:
            if not v.is_valid:
                continue
            # Соседи вдоль граничных рёбер
            neighbors = []
            for e in v.link_edges:
                if e.is_boundary:
                    other = e.other_vert(v)
                    neighbors.append(other.co.copy())

            if len(neighbors) == 2:
                avg = (neighbors[0] + neighbors[1]) / 2
                new_pos = v.co.lerp(avg, factor)
                new_pos.z = 0.0  # держим Z=0
...
def build_flat_base(context, pts, offset, smooth_iter):
    ...
    # Детализация метабола: чем меньше resolution — тем точнее контур.
    # Используем более мелкое значение для плавности.
    mb_resolution = max(offset * 0.08, 0.0001)   # было 0.15, теперь 0.08
    elem_radius   = offset * 1.1
    grid_size     = offset * 0.35
    merge_dist    = elem_radius * 0.03            # точнее merge
    ...
    # (пока is_boundary ещё работает корректно)
    if smooth_iter > 0:
        smooth_contour(bm, iterations=smooth_iter, factor=0.5)
...
def prepare_letters_mesh(bm):
    """Из меша букв оставляет контуры, заполняет с дырками, нормали вверх."""
...
class KEYCHAIN_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        L(f"=== Generate: '{text}', offset={offset}, smooth={smooth_iter}")
...
class KeychainProperties(PropertyGroup):
    ...
    smooth_iterations: IntProperty(
        ...
        description="Количество итераций сглаживания края подложки (0 = без сглаживания)",
